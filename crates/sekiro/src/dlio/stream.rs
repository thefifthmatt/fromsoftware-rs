...
use super::{DLFileSeekDirection, DLIOResult};
...
pub struct DLFileInputStream {
    ...
    _file_operator_container: usize,
...
mod test {
    ...
    #[test]
    fn proper_sizes() {
        assert_eq!(0x18, size_of::<DLFileInputStream>());
        assert_eq!(0x30, size_of::<DLMemoryInputStream>());
        assert_eq!(0x10, size_of::<DLPseudoAsyncInputStream>());
        assert_eq!(0x60, size_of::<DLBufferedInputStream>());
        assert_eq!(0x18, size_of::<DLFileOutputStream>());
        assert_eq!(0x58, size_of::<DLMemoryOutputStream>());
    }
}
