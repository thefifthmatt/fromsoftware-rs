...
impl DLCharacterSet {
    /// Returns the [encoding_rs::Encoding] that corresponds to this character set
...
fn encode_str<U: Copy>(s: &str, enc: DLCharacterSet) -> Result<Vec<U>, DLStringError> {
    ...
    match enc {
        ...
        _ => {
            let (encoded, _, errors) = enc.encoding().encode(s);
            if errors {
...
fn decode_bytes(bytes: &[u8], enc: DLCharacterSet) -> Result<Cow<'_, str>, DLStringError> {
    match enc {
        ...
        _ => {
            let (cow, _, errors) = enc.encoding().decode(bytes);
            if errors {
...
fn bytes_eq_str(bytes: &[u8], enc: DLCharacterSet, other: &str) -> bool {
    match enc {
        ...
        _ => {
            let mut decoder = enc.encoding().new_decoder();
...
///
/// For UTF-8/16 this is allocation-free.
